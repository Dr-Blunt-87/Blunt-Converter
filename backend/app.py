...
def _classify_drum_hits(drums_wav_path):
    """Läser in drums.wav, hittar varje trumslag (onset) och klassar det
    som kick/snare/hihat_open/hihat_closed/cymbal baserat på vilket
    frekvensband som har mest energi precis efter slaget."""
    y, sr = librosa.load(str(drums_wav_path), sr=None, mono=True)

    kick_band = _bandpass(y, sr, None, 150)     # golv ~35Hz i praktiken via highpass i inspelningen
    snare_band = _bandpass(y, sr, 150, 1000)
    hihat_band = _bandpass(y, sr, 5000, None)
    ...
    bands = {
        ...
        'hihat_closed': hihat_band, 'hihat_open': hihat_band, 'cymbal': hihat_band,
    }
...
def split_drums(drums_wav_path, output_folder, want_audio, want_midi, audio_format='wav'):
    ...
    if want_midi:
        ...
        drum_inst = pretty_midi.Instrument(program=0, is_drum=True, name='Drums Split')
        for t, category, s in hits:
            note_num = DRUM_GM_NOTES.get(category, 38)
            drum_inst.notes.append(pretty_midi.Note(velocity=100, pitch=note_num, start=t, end=t + 0.1))
        pm.instruments.append(drum_inst)
        midi_path = output_folder / 'drums_split.mid'
