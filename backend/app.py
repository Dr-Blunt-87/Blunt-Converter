...
def serve_frontend():
    """Serverar index.html direkt från backend, så man kan öppna
    t.ex. http://192.168.1.X:5000/ på mobilen utan att flytta filen dit."""
    return send_from_directory(BASE.parent, 'index.html')
...
def run_conversion(task_id, youtube_url, file_bytes, file_name, output_modes, audio_format, selected_stems, split_drums_flag):
    ...
    try:
        ...
        # "Bara hela låten" - inget behov av Demucs alls, bara konvertera till valt format
        if output_modes == ['original']:
            set_progress(task_id, percent=70, message='Sparar hela låten...')
            if audio_format == 'mp3':
                out_file = output_folder / f'{fname}.mp3'
                run_cmd(['ffmpeg', '-y', '-i', str(audio_path), '-codec:a', 'libmp3lame', '-qscale:a', '2', str(out_file)])
            else:
                out_file = output_folder / f'{fname}.wav'
                shutil.copy(audio_path, out_file)
            result_files.append(out_file)
            set_progress(
