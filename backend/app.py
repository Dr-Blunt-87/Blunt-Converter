...
def _normalize_peak(signal, target_peak=0.9):
    """Höjer/sänker en signal så toppen landar på target_peak (0.9 = nästan fullt,
    lite marginal kvar mot clipping). Gör att bas/gitarr/trummor osv hamnar på
    jämförbar volym istället för att vara kvar på vad Demucs råkade ge dem."""
...
def split_drums(drums_wav_path, output_folder, want_audio, want_midi, audio_format='wav'):
    ...
    if want_audio:
        ...
        if np.any(combined):
            combined = _normalize_peak(combined)
    ...
    try:
        ...
        if 'stems' in output_modes:
            for s in selected_stems:
                ...
                norm_data = _normalize_peak(norm_data)
