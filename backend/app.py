...
def split_drums(drums_wav_path, output_folder, want_audio, want_midi, audio_format='wav'):
    ...
    if want_audio:
        for cat in categories:
            # Viktigt: isolera från ORIGINALLJUDET (y), inte det smala frekvensbandet —
            # bandet användes bara för att KLASSIFICERA vilket slag som är vilket.
            # Att bygga ljudfilen från det smala bandet gör den dov/filtrerad/tunn.
            gated = np.zeros_like(y)
            for t, category, s in hits:
                if category != cat:
                    continue
                end = min(s + hit_len, len(y))
                gated[s:end] = y[s:end]
            if not np.any(gated):
                continue
